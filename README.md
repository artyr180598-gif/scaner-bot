...
Основной принцип — **PRE-MOVE first**: лучше выдать `NO TRADE`, чем показать
...
## Что изменилось в 3.7.3

RID больше не выбирает фиксированные первые 60 монет. Снимок тикеров учитывает
все активные USDT perpetual-контракты биржи, после чего быстрый 5m/15m фильтр
ищет свежий аномальный импульс и затухающий откат. Только подходящие кандидаты
получают дорогую проверку 15m/1h и микроструктуры. В Telegram отдельно видны
размер всего рынка, число быстрых проверок, предварительный список и глубокий
анализ. Неликвидные контракты и широкий спред считаются явным отказом, чтобы
расширение охвата не превращалось в неисполняемые сигналы.

## Что изменилось в 3.7.2

Версия 3.7.1 расширяет отдельный RID-поиск: весь ликвидный список проверяется
последовательными группами, модель распознаёт фазы FORMING/ARMED/ENTRY на 5m и 15m,
а Telegram показывает до трёх ближайших кандидатов без выдачи преждевременного входа.
Молодые листинги анализируются после 60 реальных 5m/15m свечей; отсутствие часовой
истории блокирует автоматическую отправку, но больше не выбрасывает монету из ручного
наблюдения. Paper-калибровка модели отделена версией `rid-mtf-pullback-2.1.0`.

## Что изменилось в 3.7.0

- добавлен полностью отдельный `/rid` и пункт меню `🧭 Стратегия RID`;
- RID не меняет PRIME-рейтинг и имеет собственные пороги, cooldown и лимит уведомлений;
- последовательность RID использует только закрытые свечи: аномальный 5m импульс,
  сокращение объёма на откате, допустимая глубина отката и закрытая свеча реактивации;
- автоматический RID строже ручного: требуется откат 30–50%, подтверждение 1h и хотя бы
  один согласованный признак OI/taker-flow/стакана;
- RID SHORT по умолчанию выключен: исходная зеркальная гипотеза не прошла отложенную
  шестимесячную проверку и не смешивается с подтверждённым LONG-ядром;
- план 50/30/20 не является мартингейлом: это части одного рассчитанного объёма с единым
  неизменяемым стопом и общим риском;
- если свежая котировка ушла, стоп шире 4.5% или TP2 хуже 1.8R после издержек, торговый
  план не создаётся;
- RID paper-результаты отделены версией `rid-long-pullback-1.0.0`; процент успеха не
  показывается до накопления 30 закрытых виртуальных сделок.
