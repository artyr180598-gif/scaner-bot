...
Если ADMIN_CHAT_IDS пуст — бот отвечает всем.
Если ALLOWED_CHAT_IDS пуст — пропускает всех (можно ограничить админами).
"""
...
class AccessControlMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        if self.s.admin_chat_ids and chat_id not in self.s.admin_chat_ids:
            # Если заданы только админы — остальным нельзя.
