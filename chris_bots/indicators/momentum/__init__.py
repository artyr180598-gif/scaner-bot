"""
Индикаторы моментума: RSI, MACD, Stochastic, CCI.
"""
...
def rsi(period: int = 14) -> Indicator:
    ...
    def signal(df: pd.DataFrame) -> SignalSeries:
        ...
        s[r < 30] = 1   # перепродан → бычий
        s[r > 70] = -1  # перекуплен → медвежий
        # Мягкая сила: 30 = сильный бычий, 50 = нейтрально.
...
def stochastic(k: int = 14, d: int = 3, smooth: int = 3) -> Indicator:
    ...
    def signal(df: pd.DataFrame) -> SignalSeries:
        ...
        zone[k_s < 20] = 1
        zone[k_s > 80] = -1
        # Берём только согласные cross + zone.
