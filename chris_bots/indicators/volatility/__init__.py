"""Индикаторы волатильности: ATR, Bollinger Bands, Keltner."""
...
def atr(period: int = 14) -> Indicator:
    ...
    def signal(df: pd.DataFrame) -> SignalSeries:
        # ATR сам по себе не даёт направления — это «мощность».
        # Но мы возвращаем 0 и используем strength=1 для обозначения наличия.
...
def bollinger(period: int = 20, std: float = 2.0) -> Indicator:
    ...
    def signal(df: pd.DataFrame) -> SignalSeries:
        ...
        s[p < 0.1] = 1   # у нижней полосы → бычий отскок
        s[p > 0.9] = -1  # у верхней → медвежий
