"""
Индикаторы объёма: OBV, VWAP, Volume Profile, аномальные свечи.

Лучшее из CryptoSignal + Freqtrade:
- OBV тренд (вверх/вниз)
- Volume Z-score: выбросы объёма (>2σ)
- Volume-confirmed price move
"""
...
def volume_zscore(window: int = 20) -> Indicator:
    """Аномальный объём: |z| > 2.

    z считается по ПРЕДЫДУЩИМ `window` барам (без утечки текущего).
    """
    ...
    return Indicator(
        ...
        group="volume",
        weight=0.8,
        description="Volume z-score (anomaly detection, no-lookahead)",
        fn=compute,
        signal_fn=signal,
    )
