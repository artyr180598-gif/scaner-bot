...
async def run_scanner_once(settings: Settings) -> bool:
    ...
    stopper.cancel()
    # Сканер завершился сам или упал (биржи недоступны уже не повод —
    # сканер живёт в деградирующем режиме и догоняет их сам); исключение
    # уйдёт супервизору, который перезапустит процесс с backoff.
    await runner
    return False
