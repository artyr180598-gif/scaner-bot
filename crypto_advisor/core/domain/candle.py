...
class Candle:
    """Одна свеча OHLCV. timestamp — ms epoch времени ОТКРЫТИЯ (как у ccxt)."""
...
@dataclass(slots=True)
class Candles:
    """Набор свечей. Удобное API: .df, .closes, .highs, .lows, .volumes, .last."""
    ...
    @classmethod
    def from_arrays(cls, closes: list, exchange: str, symbol: str, timeframe: str,
                    opens: Optional[list] = None,
                    highs: Optional[list] = None,
                    lows: Optional[list] = None,
                    volumes: Optional[list] = None) -> "Candles":
        """Синтетические/тестовые свечи из массивов цен."""
        n = len(closes)
        opens = list(opens) if opens else [closes[i - 1] for i in range(n)]
        highs = list(highs) if highs else [max(opens[i], closes[i]) * 1.001 for i in range(n)]
        lows = list(lows) if lows else [min(opens[i], closes[i]) * 0.999 for i in range(n)]
        volumes = list(volumes) if volumes else [1.0] * n
        tf_sec = _timeframe_to_seconds(timeframe)
        now = int(datetime.now(tz=timezone.utc).timestamp() * 1000)
        candles = [
            Candle(
                timestamp=now - (n - i) * tf_sec * 1000,
                open=float(opens[i]),
                high=float(highs[i]),
                low=float(lows[i]),
